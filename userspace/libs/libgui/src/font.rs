// Font Support
//
// Provides bitmap font support for text rendering.
...
pub struct Font {
    ...
    pub num_chars: u8,
    /// Glyph data (each glyph is height bytes, 1 bit per pixel)
    pub data: &'static [u8],
}
...
impl Font {
    ...
    pub fn glyph(&self, c: u8) -> &[u8] {
        let index = if c >= self.first_char && c < self.first_char + self.num_chars {
            (c - self.first_char) as usize
        } else {
            // Return space for unknown characters
            if ' ' >= self.first_char as char && (' ' as u8) < self.first_char + self.num_chars {
                (' ' as u8 - self.first_char) as usize
            } else {
                0
            }
        };
        ...
        if end <= self.data.len() {
            ...
            &self.data[0..glyph_size.min(self.data.len())]
        }
    }
}

/// Built-in 8x8 font (CP437-based, ASCII printable characters)
pub static BUILTIN_FONT: Font = Font {
    width: 8,
    height: 8,
    first_char: 32,  // Space
    num_chars: 95,   // Space through ~
    data: &FONT_8X8_DATA,
};
