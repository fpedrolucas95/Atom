...
impl From<NetError> for RecordError {
    fn from(e: NetError) -> Self { Self::Io(e) }
}
impl From<AeadError> for RecordError {
    fn from(e: AeadError) -> Self { Self::Aead(e) }
}
...
impl RecordStream {
    ...
    pub fn send_encrypted(&mut self, content_type: u8, data: &[u8]) -> Result<(), RecordError> {
        ...
        let aad = [CT_APPLICATION_DATA, 0x03, 0x03, (ct_len >> 8) as u8, ct_len as u8];
                ...
                if self.incoming.len() >= 5 + len {
                    ...
                    if ct == CT_APPLICATION_DATA {
                        if let (Some(key), Some(iv)) = (&self.read_key, &self.read_iv) {
                            ...
                            let real_type = strip_padding(&plain)?;
                            let content = plain[..plain.len() - 1].to_vec();
                            return Ok(Record { content_type: real_type, data: content });
                        }
                    }
                    return Ok(Record { content_type: ct, data: body });
                }
            }
            // Need more data
            self.fill_buf()?;
    ...
    fn fill_buf(&mut self) -> Result<(), RecordError> {
        ...
        let n = net_recv(self.netd_port, self.socket_id, &mut tmp, 30_000)
...
/// Strip zero-padding from a TLSInnerPlaintext; return the real ContentType byte.
fn strip_padding(data: &[u8]) -> Result<u8, RecordError> {
