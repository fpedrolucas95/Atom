...
/// # Safety
///
/// The caller must ensure `num` is a valid syscall number and that invoking it
/// with no arguments is valid for that syscall. Performing a syscall executes
/// arbitrary kernel-side behavior and may violate memory safety if the syscall
/// expects pointer arguments.
#[inline(always)]
...
/// # Safety
///
/// The caller must ensure `num` is a valid syscall number and that `arg0` is a
/// valid argument for it (e.g. if it represents a pointer/length, it must refer
/// to a valid, correctly-sized memory region for the operation).
#[inline(always)]
...
/// # Safety
///
/// The caller must ensure `num` is a valid syscall number and that `arg0` and
/// `arg1` are valid arguments for it (e.g. any pointer/length pair must refer to
/// a valid, correctly-sized memory region for the operation).
#[inline(always)]
...
/// # Safety
///
/// The caller must ensure `num` is a valid syscall number and that `arg0`,
/// `arg1` and `arg2` are valid arguments for it (e.g. any pointer/length
/// arguments must refer to valid, correctly-sized memory regions).
#[inline(always)]
...
/// # Safety
///
/// The caller must ensure `num` is a valid syscall number and that all
/// arguments are valid for it (e.g. any pointer/length arguments must refer to
/// valid, correctly-sized memory regions).
#[inline(always)]
