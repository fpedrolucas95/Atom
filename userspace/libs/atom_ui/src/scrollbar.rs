...
impl ScrollBar {
    ...
    fn thumb_contains(&self, px: i32, py: i32) -> bool {
        ...
        px >= tx as i32 && py >= ty as i32 && px < (tx + tw) as i32 && py < (ty + th) as i32
    }
        ...
        match event {
            MouseEvent::ButtonDown {
                ...
                if self.thumb_contains(*x, *y) {
                    self.active = true;
                    self.drag_start = if self.axis == ScrollAxis::Vertical {
                        *y
                    } else {
                        *x
                    };
            ...
            MouseEvent::Move { x, y, .. } => {
                ...
                let cursor = if self.axis == ScrollAxis::Vertical {
                    *y
                } else {
                    *x
                };
    ...
    pub fn draw(&self, surface: &mut Surface) {
        ...
        // Thumb
        let (tx, ty, tw, th) = self.thumb_rect();
        let thumb_c: Color = if self.active {
