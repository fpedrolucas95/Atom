...
impl Button {
    ...
    pub fn new(x: u32, y: u32, width: u32, height: u32, label: impl Into<String>) -> Self {
        Self { x, y, width, height, label: label.into(), style: ButtonStyle::Default,
    ...
    pub fn primary(x: u32, y: u32, width: u32, height: u32, label: impl Into<String>) -> Self {
        Self { x, y, width, height, label: label.into(), style: ButtonStyle::Primary,
               state: ButtonState::Normal }
    }

    /// Create a Danger-style button.
    pub fn danger(x: u32, y: u32, width: u32, height: u32, label: impl Into<String>) -> Self {
        Self { x, y, width, height, label: label.into(), style: ButtonStyle::Danger,
               state: ButtonState::Normal }
    }

    /// Set the button as disabled.
    pub fn disabled(mut self) -> Self { self.state = ButtonState::Disabled; self }
    ...
    pub fn handle_mouse(&mut self, event: &MouseEvent) -> WidgetEvent {
        if self.state == ButtonState::Disabled { return WidgetEvent::None; }
        ...
        match event {
            ...
            MouseEvent::ButtonUp { button: MouseButton::Left, x, y } => {
                ...
                if was_down && inside { WidgetEvent::Clicked } else { WidgetEvent::None }
            }
            _ => WidgetEvent::None,
        }
    }
    ...
    fn draw_default(&self, surface: &mut Surface, r: u32) {
        let (bg, label_color, border_color) = match self.state {
            ButtonState::Normal   => (ATOM_COLOR_SURFACE_ALT,  ATOM_COLOR_TEXT_PRIMARY,   ATOM_COLOR_BORDER),
            ButtonState::Hover    => (ATOM_COLOR_WIN_HDR,      ATOM_COLOR_TEXT_PRIMARY,   ATOM_COLOR_ACCENT),
    ...
    fn draw_primary(&self, surface: &mut Surface, r: u32) {
        ...
        let (start, end) = match self.state {
            ...
            ButtonState::Pressed => (
                Color::rgb(0x38, 0x6A, 0xCC),
                Color::rgb(0x5C, 0x42, 0xCC),
            ),
        ...
        surface.fill_rect_rounded_aa_gradient_v(
            self.x, self.y, self.width, self.height, r, start, end,
        );
    ...
    fn draw_danger(&self, surface: &mut Surface, r: u32) {
        ...
        if alpha < 255 {
            surface.fill_rect_rounded_alpha(
                self.x, self.y, self.width, self.height, r, bg_c, alpha,
            );
        } else {
            surface.fill_rect_rounded_aa(
                self.x, self.y, self.width, self.height, r, bg_c,
            );
        }
    ...
    fn draw_label(&self, surface: &mut Surface, fg: Color) {
        ...
        let ty = self.y.saturating_add(self.height.saturating_sub(text_h) / 2);
