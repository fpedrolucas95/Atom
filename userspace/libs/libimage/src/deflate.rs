...
impl HuffmanTree {
    ...
    fn decode(&self, reader: &mut BitReader<'_>) -> Option<u16> {
        ...
        for bits in 1..=(self.max_len as usize) {
            ...
            if count > 0 && code >= self.first_code[bits]
                && code < self.first_code[bits] + count
            {
...
fn make_fixed_litlen() -> HuffmanTree {
    ...
    for i in 0..=143usize  { lengths[i] = 8; }
    for i in 144..=255usize { lengths[i] = 9; }
    for i in 256..=279usize { lengths[i] = 7; }
    for i in 280..=287usize { lengths[i] = 8; }
...
fn decompress_stored(reader: &mut BitReader<'_>, out: &mut Vec<u8>) -> Result<(), ImageError> {
    reader.align_byte();
    let len  = reader.read_u16_le().ok_or(ImageError::DecompressionFailed)?;
    let nlen = reader.read_u16_le().ok_or(ImageError::DecompressionFailed)?;
    if len != !nlen {
        return Err(ImageError::DecompressionFailed);
    }
    for _ in 0..len {
...
fn decompress_block(
    ...
    loop {
        let sym = lit_tree.decode(reader).ok_or(ImageError::DecompressionFailed)?;
        match sym {
            ...
            257..=285 => {
                ...
                let extra_val = if extra > 0 {
                    reader.read_bits(extra).ok_or(ImageError::DecompressionFailed)? as u16
                } else {
                    0
                };
                let length = base_len + extra_val;
 
                let dist_code = dist_tree.decode(reader).ok_or(ImageError::DecompressionFailed)?;
                if dist_code as usize >= DIST_TABLE.len() {
                    return Err(ImageError::DecompressionFailed);
                }
                let (base_dist, extra_d) = DIST_TABLE[dist_code as usize];
                let extra_dv = if extra_d > 0 {
                    reader.read_bits(extra_d).ok_or(ImageError::DecompressionFailed)? as u16
                } else {
                    0
                };
...
fn decode_dynamic_trees(
    ...
    let mut i = 0;
    while i < total {
        let sym = cl_tree.decode(reader).ok_or(ImageError::DecompressionFailed)?;
        match sym {
            0..=15 => {
                lengths[i] = sym as u8;
                i += 1;
            ...
            16 => {
                ...
                if i == 0 { return Err(ImageError::DecompressionFailed); }
                let prev = lengths[i - 1];
                for _ in 0..rep {
                    if i >= total { return Err(ImageError::DecompressionFailed); }
                    lengths[i] = prev;
                    i += 1;
                }
            }
