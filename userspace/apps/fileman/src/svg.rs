...
impl SvgBitmap {
    ...
    pub fn blit_surface(&self, s: &SharedSurface, dx: u32, dy: u32) {
...
fn render_svg(svg: &[u8], out_w: u32, out_h: u32, px: &mut Vec<u32>) {
    ...
    // Transform stack: scaled translate in output pixels
    let mut tstk: [(i32, i32); 8] = [(0, 0); 8];
    ...
    while i < svg.len() {
        ...
        if svg[i] == b'/' {
            ...
            if &svg[ts..i] == b"g" && depth > 0 { depth -= 1; }
            while i < svg.len() && svg[i] != b'>' { i += 1; }
            i += 1;
            continue;
        }
        ...
        let ctx = cur_txy(&tstk, depth);

        match tag {
            b"g" if !self_close => {
                if depth < 8 {
                    let (dtx, dty) = parse_translate(attrs);
                    tstk[depth] = (sc(dtx, vbw, ow), sc(dty, vbh, oh));
                    depth += 1;
                }
            }
...
fn cur_txy(stk: &[(i32, i32); 8], depth: usize) -> (i32, i32) {
    let mut tx = 0i32;
    let mut ty = 0i32;
    for i in 0..depth { tx += stk[i].0; ty += stk[i].1; }
    (tx, ty)
}
...
fn elem_rect(
    ...
    let x0 = sc(x,     vbw, ow) + tx;
    let y0 = sc(y,     vbh, oh) + ty;
    let x1 = sc(x + w, vbw, ow) + tx;
    let y1 = sc(y + h, vbh, oh) + ty;
...
fn elem_circle(
    ...
    if r <= 0 { return; }
    let pcx = sc(cx, vbw, ow) + tx;
    let pcy = sc(cy, vbh, oh) + ty;
    let pr  = sc(r, vbw, ow).max(sc(r, vbh, oh));
...
fn elem_ellipse(
    ...
    let pcx = sc(cx, vbw, ow) + tx;
    let pcy = sc(cy, vbh, oh) + ty;
    let pr  = (sc(rx, vbw, ow) + sc(ry, vbh, oh)) / 2;
...
fn elem_line(
    ...
    let x1 = sc(attr_i(attrs, b"x1"), vbw, ow) + tx;
    let y1 = sc(attr_i(attrs, b"y1"), vbh, oh) + ty;
    let x2 = sc(attr_i(attrs, b"x2"), vbw, ow) + tx;
    let y2 = sc(attr_i(attrs, b"y2"), vbh, oh) + ty;
    if let Some((sc_col, sw)) = stroke_info(attrs, vbw, ow) {
...
fn elem_poly(
    px: &mut Vec<u32>, ow: i32, oh: i32, vbw: i32, vbh: i32,
    attrs: &[u8], (tx, ty): (i32, i32), closed: bool,
) {
    let raw = match get_attr(attrs, b"points") { Some(v) => v, None => return };
    let pts = parse_points(raw, vbw, vbh, ow, oh, tx, ty);
    if let Some(fc) = fill_color(attrs) {
    ...
    while i < d.len() {
        ...
        match cmd {
            b'M' => {
                ...
                sub.push((sc(cx, vbw, ow) + tx, sc(cy, vbh, oh) + ty));
...
fn fill_color(attrs: &[u8]) -> Option<u32> {
    ...
    // Check fill-opacity / opacity
    if let Some(fo) = get_attr(attrs, b"fill-opacity") {
        if is_near_zero(fo) { return None; }
    }
    if let Some(op) = get_attr(attrs, b"opacity") {
        if is_near_zero(op) { return None; }
    }

    let (r, g, b) = parse_color_str(v)?;
...
fn stroke_info(attrs: &[u8], vbw: i32, ow: i32) -> Option<(u32, i32)> {
    ...
    Some((0xFF00_0000 | ((r as u32) << 16) | ((g as u32) << 8) | b as u32, sw_px))
}

/// Returns true if the decimal string represents a value ≈ 0 (e.g. "0", "0.0", "0.01").
fn is_near_zero(v: &[u8]) -> bool {
    let mut i = 0;
    while i < v.len() && is_ws(v[i]) { i += 1; }
    if i >= v.len() || v[i] != b'0' { return false; }
    i += 1;
    if i >= v.len() || is_ws(v[i]) { return true; }
    if v[i] != b'.' { return false; }
    i += 1;
    // "0." followed by anything other than a non-zero digit → near zero
    i >= v.len() || v[i] == b'0'
}

fn parse_color_str(v: &[u8]) -> Option<(u8, u8, u8)> {
    if v.starts_with(b"#") {
        if v.len() >= 7 {
            return Some((
                (hn(v[1]) << 4) | hn(v[2]),
        ...
        if v.len() >= 4 {
            return Some((hn(v[1]) * 17, hn(v[2]) * 17, hn(v[3]) * 17));
        }
        return None;
    }
    match v {
...
fn get_attr<'a>(attrs: &'a [u8], name: &[u8]) -> Option<&'a [u8]> {
    ...
    while i < attrs.len() {
        ...
        while i < attrs.len() && is_ws(attrs[i]) { i += 1; }
        if i >= attrs.len() || attrs[i] != b'=' { continue; }
        i += 1;
        while i < attrs.len() && is_ws(attrs[i]) { i += 1; }
        if i >= attrs.len() { break; }
...
fn pn(d: &[u8], i: &mut usize) -> i32 {
    ...
    let mut v = 0i32;
    while *i < d.len() && d[*i] >= b'0' && d[*i] <= b'9' {
        v = v.saturating_mul(10).saturating_add((d[*i] - b'0') as i32);
        *i += 1;
    }
    if *i < d.len() && d[*i] == b'.' {
        *i += 1;
        while *i < d.len() && d[*i] >= b'0' && d[*i] <= b'9' { *i += 1; }
    }
    // Skip optional exponent
    if *i < d.len() && (d[*i] == b'e' || d[*i] == b'E') {
        *i += 1;
        if *i < d.len() && (d[*i] == b'+' || d[*i] == b'-') { *i += 1; }
        while *i < d.len() && d[*i] >= b'0' && d[*i] <= b'9' { *i += 1; }
    }
    if neg { -v } else { v }
}
...
fn parse_translate(attrs: &[u8]) -> (i32, i32) {
    let t = match get_attr(attrs, b"transform") { Some(v) => v, None => return (0, 0) };
    let mut i = 0;
    while i + 10 <= t.len() {
        if &t[i..i + 10] == b"translate(" {
            i += 10;
            let tx = pn(t, &mut i);
            let ty = pn(t, &mut i);
            return (tx, ty);
...
fn parse_points(
    raw: &[u8], vbw: i32, vbh: i32, ow: i32, oh: i32, tx: i32, ty: i32,
) -> Vec<(i32, i32)> {
    let mut pts = Vec::new();
    let mut i = 0;
    while i < raw.len() {
        ...
        let x = pn(raw, &mut i);
        let y = pn(raw, &mut i);
        pts.push((sc(x, vbw, ow) + tx, sc(y, vbh, oh) + ty));
    }
    pts
}
