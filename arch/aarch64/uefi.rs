// AArch64 UEFI Boot Entry and Firmware Handoff
//
...
type EfiGetMemoryMap = extern "C" fn(
...
type EfiAllocatePool = extern "C" fn(
    pool_type: u32,
    size: usize,
    buffer: *mut *mut c_void,
) -> EfiStatus;

type EfiFreePool = extern "C" fn(buffer: *mut c_void) -> EfiStatus;

type EfiExitBootServices =
    extern "C" fn(image_handle: EfiHandle, map_key: usize) -> EfiStatus;

type EfiWaitForEvent =
    extern "C" fn(number_of_events: usize, events: *mut *mut c_void, index: *mut usize)
        -> EfiStatus;

type EfiStall = extern "C" fn(microseconds: usize) -> EfiStatus;

type EfiLocateProtocol = extern "C" fn(
    protocol: *const EfiGuid,
    registration: *mut c_void,
    interface: *mut *mut c_void,
) -> EfiStatus;

type EfiSetWatchdogTimer = extern "C" fn(
    timeout: usize,
    watchdog_code: u64,
    data_size: usize,
...
type EfiFileClose = extern "C" fn(this: *mut EfiFileProtocol) -> EfiStatus;

type EfiFileRead = extern "C" fn(
    this: *mut EfiFileProtocol,
    buffer_size: *mut usize,
    buffer: *mut c_void,
) -> EfiStatus;

type EfiFileGetInfo = extern "C" fn(
    this: *mut EfiFileProtocol,
...
type EfiOpenVolume = extern "C" fn(
...
type EfiHandleProtocol = extern "C" fn(
    handle: EfiHandle,
    protocol: *const EfiGuid,
    interface: *mut *mut c_void,
) -> EfiStatus;
...
fn get_cpu_vendor() -> [u8; 12] {
    ...
    let v = b"ARM64-GEN";
    vendor[..v.len()].copy_from_slice(v);
    vendor
}

fn get_cpu_brand() -> [u8; 48] {
    let mut brand = [0u8; 48];
    let b = b"Generic AArch64 Processor";
    brand[..b.len()].copy_from_slice(b);
    brand
}
...
#[no_mangle]
pub extern "C" fn efi_main(image: EfiHandle, system_table: *mut c_void) -> EfiStatus {
    ...
    loop {
        ...
        if mmap_buf.is_null() || mmap_buf_size < alloc_size {
            ...
            if st_alloc != EFI_SUCCESS || new_buf.is_null() {
                cleanup_pool(bs, &mut mmap_buf);
                return if st_alloc == EFI_SUCCESS { EFI_INVALID_PARAMETER } else { st_alloc };
            }
            mmap_buf = new_buf;
        }
