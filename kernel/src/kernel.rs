...
pub unsafe extern "C" fn kmain(boot_info: &'static BootInfo) -> ! {
    ...
    if boot_info.framebuffer_present {
        ...
        if mm::vm::map_framebuffer(fb.address, fb.size) {
            graphics::init(fb);
            // Note: Graphics terminal removed. All UI rendering is now in userspace.
            // Early boot messages use serial output only.
        }
    }
    ...
    input::init();
    // Note: PS/2 mouse initialization is now done in userspace.
    // The kernel only provides IRQ buffering; userspace drivers handle device setup.
    ...
    // Note: UI services are now launched entirely in userspace.
    // The init process is responsible for starting the desktop environment,
    // input drivers, and other userspace services via the service manager.
    // The kernel does not contain any shell, UI, or graphics rendering logic
    // beyond minimal framebuffer mapping for userspace access.
